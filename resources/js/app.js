...
(function ($) {
    ...
    $(document).on('click', '[data-sidebar-target]', function () {
        ...
        if ($('.sidebar-group').hasClass('show')) {
            $('.sidebar-group .sidebar' + target).niceScroll({overflowx: false});
        } else {
    ...
    if (wind_.width() >= 1200) {
        ...
        setTimeout(function () {
            $('.card-scroll').niceScroll({overflowx: false});
            $('.dropdown-scroll').niceScroll({overflowx: false});
            $('.table-responsive').niceScroll();

            $('.app-block .app-content .app-lists').niceScroll({overflowx: false});
            $('.app-block .app-sidebar').niceScroll({overflowx: false});
            $('.chat-block .chat-sidebar .chat-sidebar-content').niceScroll({overflowx: false});
            var chat_messages = $('.chat-block .chat-content .messages');
            if (chat_messages.length) {
                chat_messages.niceScroll({overflowx: false});
    ...
    $(document).on('mouseleave', 'body.small-navigation .navigation', function (e) {
        ...
        $(this).find('.navigation-menu-body ul')
            .prev('a')
            .find('.sub-menu-arrow')
            .removeClass('rotate-in ti-minus')
            .addClass('ti-plus');
    });
